...
data class TeacherCoTeachingItem(
    val id: String,
    val subjectTitle: String,
    val teachersAndRoles: String,
    val classAndDetails: String,
    val statusText: String = "Firmata"
)

@OptIn(ExperimentalMaterial3Api::class)
@Composable
fun TeacherCoTeachingScreen(
    sessions: List<TeacherCoTeachingItem> = emptyList(),
    onBack: () -> Unit = {}
) {
    Scaffold(
        topBar = {
                    ...
                    Card(modifier = Modifier.fillMaxWidth(), shape = RoundedCornerShape(12.dp)) {
                        Column(modifier = Modifier.padding(14.dp), verticalArrangement = Arrangement.spacedBy(6.dp)) {
                            Row(
                                ...
                                Text(s.subjectTitle, fontWeight = FontWeight.Bold)
                                Surface(color = Color(0xFF10B981), shape = RoundedCornerShape(6.dp)) {
                                    Text(s.statusText, color = Color.White, modifier = Modifier.padding(horizontal = 6.dp, vertical = 2.dp), fontSize = 10.sp)
                                }
                            }
                            Text(s.teachersAndRoles, fontSize = 12.sp, color = MaterialTheme.colorScheme.primary, fontWeight = FontWeight.SemiBold)
                            Text(s.classAndDetails, fontSize = 12.sp, color = Color.Gray)
                        }
                    }
                }
            }
