...
data class TeacherGroupItem(
    val id: String,
    val title: String,
    val teacherAndSchedule: String,
    val roomAndRegister: String,
    val countBadgeText: String = "18 Studenti"
)

@OptIn(ExperimentalMaterial3Api::class)
@Composable
fun TeacherGroupsScreen(
    groups: List<TeacherGroupItem> = emptyList(),
    onBack: () -> Unit = {}
) {
    Scaffold(
        topBar = {
                ...
                items(groups) { item ->
                    Card(modifier = Modifier.fillMaxWidth(), shape = RoundedCornerShape(12.dp)) {
                        Column(modifier = Modifier.padding(14.dp)) {
                            ...
                            Text(item.teacherAndSchedule, fontSize = 12.sp, color = MaterialTheme.colorScheme.primary, fontWeight = FontWeight.SemiBold, modifier = Modifier.padding(vertical = 4.dp))
                            Text(item.roomAndRegister, fontSize = 12.sp, color = Color.Gray)
                        }
                    }
                }
            }
