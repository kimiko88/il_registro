...
data class TeacherLeaveItem(
    ...
    val dateTimeDetails: String,
    val substituteAndProtocol: String,
    val statusText: String = "Approvato"
)

@OptIn(ExperimentalMaterial3Api::class)
@Composable
fun TeacherLeavesScreen(
    leaves: List<TeacherLeaveItem> = emptyList(),
    onBack: () -> Unit = {}
) {
    Scaffold(
        topBar = {
                    ...
                    Card(modifier = Modifier.fillMaxWidth(), shape = RoundedCornerShape(12.dp)) {
                        Column(modifier = Modifier.padding(14.dp)) {
                            Row(
                                ...
                                Text(item.leaveTitle, fontWeight = FontWeight.Bold)
                            ...
                            Text(item.dateTimeDetails, fontSize = 12.sp, color = MaterialTheme.colorScheme.primary, fontWeight = FontWeight.SemiBold, modifier = Modifier.padding(vertical = 4.dp))
                            Text(item.substituteAndProtocol, fontSize = 12.sp, color = Color.Gray)
                        }
                    }
                }
            }
