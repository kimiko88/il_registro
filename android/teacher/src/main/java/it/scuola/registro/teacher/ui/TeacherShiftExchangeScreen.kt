...
data class TeacherShiftExchangeItem(
    val id: String,
    val title: String,
    val subjectAndDates: String,
    val hoursAndStatus: String,
    val statusText: String = "Approvato"
)

@OptIn(ExperimentalMaterial3Api::class)
@Composable
fun TeacherShiftExchangeScreen(
    exchanges: List<TeacherShiftExchangeItem> = emptyList(),
    onBack: () -> Unit = {}
) {
    Scaffold(
        topBar = {
                ...
                items(exchanges) { item ->
                    Card(modifier = Modifier.fillMaxWidth(), shape = RoundedCornerShape(12.dp)) {
                        Column(modifier = Modifier.padding(14.dp), verticalArrangement = Arrangement.spacedBy(6.dp)) {
                            ...
                            Text(item.subjectAndDates, fontSize = 12.sp, color = MaterialTheme.colorScheme.primary, fontWeight = FontWeight.SemiBold)
                            Text(item.hoursAndStatus, fontSize = 12.sp, color = Color.Gray)
                        }
                    }
                }
            }
