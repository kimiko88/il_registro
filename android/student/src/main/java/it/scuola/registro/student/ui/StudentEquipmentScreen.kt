...
data class StudentEquipmentItem(
    val id: String,
...
fun StudentEquipmentScreen(
    equipment: List<StudentEquipmentItem> = emptyList(),
    onBack: () -> Unit = {}
) {
    Scaffold(
        topBar = {
                ...
                items(equipment) { item ->
                    Card(modifier = Modifier.fillMaxWidth(), shape = RoundedCornerShape(12.dp)) {
                        Row(
                            ...
                            Text(item.resourceName, fontWeight = FontWeight.SemiBold)
                            Text(
                                item.details,
                                fontWeight = FontWeight.Bold,
                                color = if (item.isHighlighted) MaterialTheme.colorScheme.primary else Color.DarkGray
                            )
                        }
                    }
                }
