...
data class StudentGoalItem(
    val id: String,
    val title: String,
    val subjectAndTarget: String,
    val progress: Float,
...
@Composable
fun StudentGoalsScreen(
    goals: List<StudentGoalItem> = emptyList(),
    onBack: () -> Unit = {}
) {
    Scaffold(
        topBar = {
                        ...
                        Column(modifier = Modifier.padding(14.dp)) {
                            Row(
                                ...
                                Surface(color = Color(0xFF3B82F6), shape = RoundedCornerShape(6.dp)) {
                                    Text(goal.progressText, color = Color.White, modifier = Modifier.padding(horizontal = 6.dp, vertical = 2.dp), fontSize = 10.sp)
                                }
                            }
                            Text(goal.subjectAndTarget, fontSize = 12.sp, color = MaterialTheme.colorScheme.primary, fontWeight = FontWeight.SemiBold, modifier = Modifier.padding(vertical = 4.dp))
                            LinearProgressIndicator(
                                progress = goal.progress,
                                modifier = Modifier.fillMaxWidth().padding(vertical = 8.dp),
                                color = MaterialTheme.colorScheme.primary
                            )
                            Text(goal.teacherFeedback, fontSize = 12.sp, color = Color.Gray)
                        }
                    }
                }
            }
