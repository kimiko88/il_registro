...
data class SecretaryInventoryAssetItem(
    val id: String,
    val title: String,
    val inventoryNumberAndCategory: String,
    val locationAndFunding: String,
    val statusText: String = "Inventariato"
)

@OptIn(ExperimentalMaterial3Api::class)
@Composable
fun SecretaryInventoryScreen(
    items: List<SecretaryInventoryAssetItem> = emptyList(),
    onBack: () -> Unit = {}
) {
    Scaffold(
        topBar = {
                ...
                items(items) { item ->
                    Card(modifier = Modifier.fillMaxWidth(), shape = RoundedCornerShape(12.dp)) {
                        Column(modifier = Modifier.padding(14.dp), verticalArrangement = Arrangement.spacedBy(6.dp)) {
                            ...
                            Text(item.inventoryNumberAndCategory, fontSize = 12.sp, color = MaterialTheme.colorScheme.primary, fontWeight = FontWeight.SemiBold)
                            Text(item.locationAndFunding, fontSize = 12.sp, color = Color.Gray)
                        }
                    }
                }
            }
