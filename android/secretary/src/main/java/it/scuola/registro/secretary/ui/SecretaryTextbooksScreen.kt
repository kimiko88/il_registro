...
data class SecretaryTextbookClassItem(
    val id: String,
    val classNameAndTrack: String,
    val costDetails: String,
    val adoptionDetails: String,
    val statusText: String = "Entro il Limite"
)

@OptIn(ExperimentalMaterial3Api::class)
@Composable
fun SecretaryTextbooksScreen(
    classes: List<SecretaryTextbookClassItem> = emptyList(),
    onBack: () -> Unit = {}
) {
    Scaffold(
        topBar = {
                    ...
                    Card(modifier = Modifier.fillMaxWidth(), shape = RoundedCornerShape(12.dp)) {
                        Column(modifier = Modifier.padding(14.dp)) {
                            Row(
                                ...
                                Text(item.classNameAndTrack, fontWeight = FontWeight.Bold)
                            ...
                            Text(item.costDetails, fontSize = 12.sp, color = MaterialTheme.colorScheme.primary, fontWeight = FontWeight.SemiBold, modifier = Modifier.padding(vertical = 4.dp))
                            Text(item.adoptionDetails, fontSize = 12.sp, color = Color.Gray)
                        }
                    }
                }
            }
