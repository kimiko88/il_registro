...
data class SecretaryExamCommitteeItem(
    val id: String,
    val title: String,
    val presidentDetails: String,
    val membersAndCandidates: String,
    val statusText: String = "Insediata"
)

@OptIn(ExperimentalMaterial3Api::class)
@Composable
fun SecretaryExamCommitteeScreen(
    committees: List<SecretaryExamCommitteeItem> = emptyList(),
    onBack: () -> Unit = {}
) {
    Scaffold(
        topBar = {
                ...
                items(committees) { item ->
                    Card(modifier = Modifier.fillMaxWidth(), shape = RoundedCornerShape(12.dp)) {
                        Column(modifier = Modifier.padding(14.dp), verticalArrangement = Arrangement.spacedBy(6.dp)) {
                            ...
                            Text(item.presidentDetails, fontSize = 12.sp, color = MaterialTheme.colorScheme.primary, fontWeight = FontWeight.SemiBold)
                            Text(item.membersAndCandidates, fontSize = 12.sp, color = Color.Gray)
                        }
                    }
                }
            }
