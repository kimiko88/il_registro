...
data class SecretaryTimetableItem(
    val id: String,
    val title: String,
    val structureDetails: String,
    val teacherDetails: String,
    val statusText: String = "Pubblicato"
)

@OptIn(ExperimentalMaterial3Api::class)
@Composable
fun SecretaryTimetableScreen(
    timetables: List<SecretaryTimetableItem> = emptyList(),
    onBack: () -> Unit = {}
) {
    Scaffold(
        topBar = {
                ...
                items(timetables) { item ->
                    Card(modifier = Modifier.fillMaxWidth(), shape = RoundedCornerShape(12.dp)) {
                        Column(modifier = Modifier.padding(14.dp)) {
                            ...
                            Text(item.structureDetails, fontSize = 12.sp, color = MaterialTheme.colorScheme.primary, fontWeight = FontWeight.SemiBold, modifier = Modifier.padding(vertical = 4.dp))
                            Text(item.teacherDetails, fontSize = 12.sp, color = Color.Gray)
                        }
                    }
                }
            }
