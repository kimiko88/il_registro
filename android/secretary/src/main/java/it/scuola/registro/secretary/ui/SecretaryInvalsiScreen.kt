...
data class SecretaryInvalsiSessionItem(
    val id: String,
    val title: String,
    val studentsAndLabs: String,
    val proctorsAndStatus: String,
    val statusText: String = "Completata"
)

@OptIn(ExperimentalMaterial3Api::class)
@Composable
fun SecretaryInvalsiScreen(
    sessions: List<SecretaryInvalsiSessionItem> = emptyList(),
    onBack: () -> Unit = {}
) {
    Scaffold(
        topBar = {
                ...
                items(sessions) { item ->
                    Card(modifier = Modifier.fillMaxWidth(), shape = RoundedCornerShape(12.dp)) {
                        Column(modifier = Modifier.padding(14.dp), verticalArrangement = Arrangement.spacedBy(6.dp)) {
                            ...
                            Text(item.studentsAndLabs, fontSize = 12.sp, color = MaterialTheme.colorScheme.primary, fontWeight = FontWeight.SemiBold)
                            Text(item.proctorsAndStatus, fontSize = 12.sp, color = Color.Gray)
                        }
                    }
                }
            }
