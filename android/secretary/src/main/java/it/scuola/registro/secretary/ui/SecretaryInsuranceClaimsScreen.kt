...
data class SecretaryInsuranceClaimItem(
    val id: String,
    val title: String,
    val injuredAndDate: String,
    val protocolsAndPolicy: String,
    val statusText: String = "Inviato a INAIL"
)

@OptIn(ExperimentalMaterial3Api::class)
@Composable
fun SecretaryInsuranceClaimsScreen(
    claims: List<SecretaryInsuranceClaimItem> = emptyList(),
    onBack: () -> Unit = {}
) {
    Scaffold(
        topBar = {
                ...
                items(claims) { item ->
                    Card(modifier = Modifier.fillMaxWidth(), shape = RoundedCornerShape(12.dp)) {
                        Column(modifier = Modifier.padding(14.dp), verticalArrangement = Arrangement.spacedBy(6.dp)) {
                            ...
                            Text(item.injuredAndDate, fontSize = 12.sp, color = MaterialTheme.colorScheme.primary, fontWeight = FontWeight.SemiBold)
                            Text(item.protocolsAndPolicy, fontSize = 12.sp, color = Color.Gray)
                        }
                    }
                }
            }
