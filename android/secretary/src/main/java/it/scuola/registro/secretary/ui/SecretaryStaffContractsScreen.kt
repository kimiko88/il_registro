...
data class SecretaryStaffContractItem(
    val id: String,
    val title: String,
    val classAndHours: String,
    val periodAndSidi: String,
    val statusText: String = "Trasmesso a NoiPA"
)

@OptIn(ExperimentalMaterial3Api::class)
@Composable
fun SecretaryStaffContractsScreen(
    contracts: List<SecretaryStaffContractItem> = emptyList(),
    onBack: () -> Unit = {}
) {
    Scaffold(
        topBar = {
                ...
                items(contracts) { item ->
                    Card(modifier = Modifier.fillMaxWidth(), shape = RoundedCornerShape(12.dp)) {
                        Column(modifier = Modifier.padding(14.dp), verticalArrangement = Arrangement.spacedBy(6.dp)) {
                            ...
                            Text(item.classAndHours, fontSize = 12.sp, color = MaterialTheme.colorScheme.primary, fontWeight = FontWeight.SemiBold)
                            Text(item.periodAndSidi, fontSize = 12.sp, color = Color.Gray)
                        }
                    }
                }
            }
