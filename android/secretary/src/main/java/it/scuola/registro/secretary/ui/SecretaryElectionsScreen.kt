...
data class SecretaryElectionItem(
    val id: String,
    val title: String,
    val turnoutAndLists: String,
    val closingAndCrypto: String,
    val statusText: String = "Seggio Aperto"
)

@OptIn(ExperimentalMaterial3Api::class)
@Composable
fun SecretaryElectionsScreen(
    elections: List<SecretaryElectionItem> = emptyList(),
    onBack: () -> Unit = {}
) {
    Scaffold(
        topBar = {
                ...
                items(elections) { item ->
                    Card(modifier = Modifier.fillMaxWidth(), shape = RoundedCornerShape(12.dp)) {
                        Column(modifier = Modifier.padding(14.dp), verticalArrangement = Arrangement.spacedBy(6.dp)) {
                            ...
                            Text(item.turnoutAndLists, fontSize = 12.sp, color = MaterialTheme.colorScheme.primary, fontWeight = FontWeight.SemiBold)
                            Text(item.closingAndCrypto, fontSize = 12.sp, color = Color.Gray)
                        }
                    }
                }
            }
