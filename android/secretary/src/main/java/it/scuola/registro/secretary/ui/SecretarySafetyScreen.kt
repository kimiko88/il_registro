...
data class SecretarySafetyDrillItem(
    val id: String,
    val title: String,
    val timeDetails: String,
    val peopleAndReport: String,
    val statusText: String = "Riuscita"
)

@OptIn(ExperimentalMaterial3Api::class)
@Composable
fun SecretarySafetyScreen(
    drills: List<SecretarySafetyDrillItem> = emptyList(),
    onBack: () -> Unit = {}
) {
    Scaffold(
        topBar = {
                ...
                items(drills) { item ->
                    Card(modifier = Modifier.fillMaxWidth(), shape = RoundedCornerShape(12.dp)) {
                        Column(modifier = Modifier.padding(14.dp), verticalArrangement = Arrangement.spacedBy(6.dp)) {
                            ...
                            Text(item.timeDetails, fontSize = 12.sp, color = MaterialTheme.colorScheme.primary, fontWeight = FontWeight.SemiBold)
                            Text(item.peopleAndReport, fontSize = 12.sp, color = Color.Gray)
                        }
                    }
                }
            }
