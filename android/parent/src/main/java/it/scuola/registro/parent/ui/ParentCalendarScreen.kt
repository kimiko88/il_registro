...
data class ParentCalendarEventItem(
    val id: String,
    val title: String,
    val dateText: String,
    val details: String,
...
fun ParentCalendarScreen(
    events: List<ParentCalendarEventItem> = emptyList(),
    onBack: () -> Unit = {}
) {
    Scaffold(
        topBar = {
                        ...
                        Column(modifier = Modifier.padding(14.dp)) {
                            Row(
                                ...
                                Surface(color = Color(0xFF3B82F6), shape = RoundedCornerShape(6.dp)) {
                                    Text(ev.tag, color = Color.White, modifier = Modifier.padding(horizontal = 6.dp, vertical = 2.dp), fontSize = 10.sp)
                                }
                            }
                            Text(ev.dateText, fontSize = 12.sp, color = MaterialTheme.colorScheme.primary, fontWeight = FontWeight.SemiBold, modifier = Modifier.padding(vertical = 4.dp))
                            Text(ev.details, fontSize = 12.sp, color = Color.Gray)
                        }
                    }
                }
            }
