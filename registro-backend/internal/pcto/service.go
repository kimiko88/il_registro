...
func (s *service) ApproveHours(ctx context.Context, actorID, actorRole, logID string, approved bool) error {
	...
	hourLog, err := s.repo.GetHourLogByID(ctx, logID)
	if err == nil && hourLog != nil {
		part, err := s.repo.GetParticipationByID(ctx, hourLog.ParticipationID)
		if err == nil && part != nil {
			project, err := s.repo.GetProjectByID(ctx, part.ProjectID)
			if err == nil && project != nil {
				if actorRole == "teacher" || actorRole == "tutor" {
					if (project.SchoolTutorID == nil || *project.SchoolTutorID != actorID) && project.CreatedBy != actorID {
						return errors.New("unauthorized: non sei il tutor scolastico assegnato a questo progetto")
					}
				}
			}
		}
