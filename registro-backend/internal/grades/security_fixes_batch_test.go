...
func TestBatchCreateGrades_DeduplicationKey(t *testing.T) {
	...
	repo.On("BatchCreate", mock.MatchedBy(func(g []*Grade) bool {
		return len(g) == 3
	})).Return(nil).Once()

	err := svc.BatchCreateGrades("teacher-1", "teacher", "school-1", gradesList)
...
func TestExport_TeacherScopingSecurity(t *testing.T) {
	...
	repo.On("FindWithFilter", mock.MatchedBy(func(f GradeFilter) bool {
		return f.TeacherID == "teacher-authenticated"
	})).Return([]Grade{}, nil).Once()

	_, _, err := svc.Export("teacher-authenticated", "school-1", filter, "json")
	assert.NoError(t, err)
	repo.AssertExpectations(t)
}
...
func TestGetMyTrend_UnavailableClassAveragePosition(t *testing.T) {
	...
	repo.On("FindByStudent", studentID).Return([]Grade{
		{
			StudentID:   studentID,
			SubjectID:   subjectID,
			GradeValue:  7.5,
...
func TestBulkImport_TeacherAssignmentEnforcement(t *testing.T) {
	...
	res, err := svc.BulkImport(teacherID, schoolID, r, 1)
	assert.NoError(t, err)
	assert.NotNil(t, res)
