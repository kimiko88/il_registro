...
func TestFindByStudent_QueryStructure(t *testing.T) {
	...
	mockRepo.On("FindByStudent", "user-or-stud-id").Return(expectedGrades, nil).Once()

	res, err := mockRepo.FindByStudent("user-or-stud-id")
...
func TestFindWithFilterPaginated_TeacherIDAndSchoolID(t *testing.T) {
	...
	mockRepo.On("FindWithFilterPaginated", filter).Return(expectedGrades, 1, nil).Once()

	res, count, err := mockRepo.FindWithFilterPaginated(filter)
	assert.NoError(t, err)
...
func TestGetWeightConfigs_Ordering(t *testing.T) {
	...
	mockRepo.On("GetWeightConfigs", schoolID, subjectID, classID).
		Return([]GradeWeightConfig{specificSubject, genericSchool}, nil).Once()

	cfgs, err := mockRepo.GetWeightConfigs(schoolID, subjectID, classID)
	assert.NoError(t, err)
	assert.Len(t, cfgs, 2)
