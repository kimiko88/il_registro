...
Il sistema usa un modello **RBAC** con 6 ruoli. La registrazione di nuovi utenti richiede autenticazione: solo `superadmin`, `admin` e `segreteria` possono creare account, secondo questa matrice:
...
|---|---|
| `POST /auth/register` | `superadmin`, `admin`, `segreteria` (JWT richiesto) |
