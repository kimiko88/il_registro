...
DROP POLICY IF EXISTS qs_revoke_only ON qualified_signatures;
...
DROP POLICY IF EXISTS ap_insert_update_unsealed ON archive_packages;
CREATE POLICY ap_insert_update_unsealed ON archive_packages
    AS PERMISSIVE FOR ALL TO PUBLIC
    USING (is_sealed = FALSE)
    WITH CHECK (is_sealed = FALSE);

DROP POLICY IF EXISTS ap_select_all ON archive_packages;
