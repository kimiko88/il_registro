-- Migration 104: Registro Visitatori, Uscite Anticipate, Segnalazioni Guasti
-- Package: visitors
...
CREATE INDEX IF NOT EXISTS idx_visitors_school_entry
    ON visitors(school_id, entry_time);
...
CREATE INDEX IF NOT EXISTS idx_early_exits_school_exit
    ON student_early_exits(school_id, exit_time);
...
DROP POLICY IF EXISTS visitors_policy ON visitors;
CREATE POLICY visitors_policy ON visitors USING (true);

ALTER TABLE student_early_exits ENABLE ROW LEVEL SECURITY;
DROP POLICY IF EXISTS student_early_exits_policy ON student_early_exits;
CREATE POLICY student_early_exits_policy ON student_early_exits USING (true);
...
CREATE POLICY maintenance_reports_policy ON maintenance_reports USING (true);
