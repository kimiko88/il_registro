...
describe('gradeUtils — Italian Grade Conversion & Formatting', () => {
  ...
  describe('gradeToNumeric', () => {
    ...
    it('handles special grade labels (A, NC, E, S, INS, O, B, D, Eccellente, Avanzato, Mediocre, Iniziale, Quasi Sufficiente)', () => {
      ...
      expect(gradeToNumeric('INS')).toBe(4.5)
      expect(gradeToNumeric('INSUFFICIENTE')).toBe(4.5)
      ...
      expect(gradeToNumeric('QUASI SUFFICIENTE')).toBe(5.5)
      expect(gradeToNumeric('MEDIOCRE')).toBe(5.5)
      expect(gradeToNumeric('INIZIALE')).toBe(5)
      expect(gradeToNumeric('GRAVEMENTE INSUFFICIENTE')).toBe(3)
    })

    it('rejects out of bounds grades outside [1.0, 10.0] scale and invalid ranges', () => {
      ...
      expect(gradeToNumeric('0-5')).toBeNull()
      expect(gradeToNumeric('1-10')).toBeNull()
