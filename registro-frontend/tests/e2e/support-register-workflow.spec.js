...
// ─────────────────────────────────────────────────────────────────────────────
// Suite 1 – Support Register (Sostegno & PEI) Workflow
// ─────────────────────────────────────────────────────────────────────────────
describe('Support Register & Special Needs Workflow', () => {
    beforeEach(() => { vi.clearAllMocks() })

    it('SR01 — SupportRegister page mounts and displays title banner', async () => {
        const { default: SupportRegister } = await import('@/pages/teacher/SupportRegister.vue')
        const wrapper = shallowMount(SupportRegister, {
            global: { plugins: [buildPinia('teacher')] }
        })
        expect(wrapper.exists()).toBe(true)
    })

    it('SR02 — SupportRegister renders action buttons for new goal and diary entry', async () => {
        const { default: SupportRegister } = await import('@/pages/teacher/SupportRegister.vue')
        const wrapper = shallowMount(SupportRegister, {
            global: { plugins: [buildPinia('teacher')] }
        })
    ...
    it('SR03 — SupportRegister displays student filter and tab navigation', async () => {
        const { default: SupportRegister } = await import('@/pages/teacher/SupportRegister.vue')
        const wrapper = shallowMount(SupportRegister, {
            global: { plugins: [buildPinia('teacher')] }
        })
