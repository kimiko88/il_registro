<template>
  <q-page class="q-pa-md bg-grey-1">
    ...
    <div class="row items-center justify-between q-mb-md">
      <div>
        ...
        <div class="text-caption text-grey">
          Sfoglia e scarica le dispense e i materiali di studio per 
    ...
    <q-card v-if="!selectedChild" class="text-center q-pa-lg bg-warning text-white">
      Seleziona un figlio dal menu in alto per visualizzare il materiale didattico
    </q-card>
    ...
    <div v-else>
      ...
      <div v-else>
        <q-card v-if="filteredMaterials.length === 0" class="text-center q-pa-xl text-grey-6 shadow-1">
          ...
          <div class="text-caption">Non è stato ancora condiviso alcun file per la classe di tuo figlio</div>
        </q-card>
...
const fetchSubjects = async () => {
  try {
    const schoolId = selectedChild.value?.school_id || selectedChild.value?.schoolId
    if (!schoolId) return
